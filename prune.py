...
def prune_prototypes(dataloader,
    ...
    if not prototypes_to_keep:
        ...
        log("prototype to keep is selected to be {}".format(prototypes_to_keep))
        prototypes_to_prune = list(set(range(original_num_prototypes)) - set(prototypes_to_keep))

    ### bookkeeping of prototypes to be pruned
    class_of_prototypes_to_prune = \
        torch.argmax(prototype_network_parallel.module.prototype_class_identity[prototypes_to_prune], dim=1).numpy().reshape(-1, 1)
