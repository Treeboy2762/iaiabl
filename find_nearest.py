        ...
        if full_save:
            ...
            for i, patch in enumerate(heaps[j]):
                ...
                plt.imsave(fname=os.path.join(dir_for_saving_images,
                           ...
                           arr=patch.original_img[:,:,0:3],
                           vmin=0.0,
                           vmax=1.0)

                # save the original image with the fa
                try:
                    if patch.original_img.shape[2]==4:
                        plt.imsave(fname=os.path.join(dir_for_saving_images,
                                                    'nearest-' + str(i+1) + '_original_with_fa.png'),
                                arr=patch.original_img[:,:,1:4],
                                vmin=0.0,
                                vmax=1.0)
                except Exception as e:
                    print("Exception in original img with fa: ", e)
                ...
                heatmap = heatmap[...,::-1]
                overlayed_original_img = 0.5 * patch.original_img[:,:,0:3] + 0.3 * heatmap
                plt.imsave(fname=os.path.join(dir_for_saving_images,
                ...
                # overlay activation on original image with fa and save the result
                try:
                    if patch.original_img.shape[2]==4:
                        img_size = patch.original_img.shape[0]
                        upsampled_act_pattern = cv2.resize(patch.act_pattern,
                        ...
                        heatmap = np.float32(heatmap) / 255
                        heatmap = heatmap[...,::-1]
                        overlayed_original_img = 0.5 * patch.original_img[:,:,1:4] + 0.3 * heatmap
                        plt.imsave(fname=os.path.join(dir_for_saving_images,
                                                    'nearest-' + str(i+1) + '_original_with_fa_and_heatmap.png'),
                                arr=overlayed_original_img,
                                vmin=0.0,
                                vmax=1.0)
                except Exception as e:
                    print("Exception in overlay activation on original img with fa: ", e)
                ...
                if patch.patch.shape[0] != img_size or patch.patch.shape[1] != img_size:
                    ...
                    overlayed_patch = overlayed_original_img[patch.patch_indices[0]:patch.patch_indices[1],
                                                             patch.patch_indices[2]:patch.patch_indices[3], 0:3]
                    plt.imsave(fname=os.path.join(dir_for_saving_images,
                ...
                high_act_patch = patch.original_img[high_act_patch_indices[0]:high_act_patch_indices[1],
                                                    high_act_patch_indices[2]:high_act_patch_indices[3], 0:3]
                np.save(os.path.join(dir_for_saving_images,
                ...
                imsave_with_bbox(fname=os.path.join(dir_for_saving_images,
                                 ...
                                 img_rgb=patch.original_img[:,:,0:3],
